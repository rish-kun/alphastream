...
content = content.replace('''    async def test_reanalyze_dispatches_existing_articles(
        ...
        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.api.v1.sentiment.ReanalysisStatusService") as MockService:
            mock_send_task.return_value = MagicMock(id="task-123")
            instance = MockService.return_value
            instance.start_reanalysis = AsyncMock(return_value=None)
        ...
        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.services.reanalysis_status.ReanalysisStatusService.start_reanalysis") as mock_start:
            mock_send_task.return_value = MagicMock(id="task-123")
