...
class TestSectorSentiment:
    ...
    async def test_sectors_with_data(self, client: AsyncClient, mock_db: AsyncMock):
        ...
        # Second call: top stocks query for all returned sectors
        mock_db.execute.side_effect = [
            MockResult(data=[sector_row]),  # sector query
            MockResult(data=[("Banking & Finance", "HDFCBANK"), ("Banking & Finance", "ICICIBANK")]),  # top stocks query (sector, ticker)
        ]
