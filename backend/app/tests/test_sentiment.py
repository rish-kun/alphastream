...
class TestSectorSentiment:
    async def test_returns_sectors(self, client: AsyncClient, mock_db: AsyncMock):
        # Mock: main sector query returns no rows -> empty list, and top stocks returns empty list
        mock_db.execute.side_effect = [
            MockResult(data=[]),  # sector query
            MockResult(data=[]),  # top stocks query
        ]
    ...
    async def test_sectors_with_data(self, client: AsyncClient, mock_db: AsyncMock):
        ...
        # Second call: top stocks query returns the top stocks across sectors
        mock_db.execute.side_effect = [
...
class TestSentimentReanalysis:
    async def test_reanalyze_dispatches_existing_articles(
        ...
        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.api.v1.sentiment.reanalysis_status_service.start_reanalysis") as mock_status:
