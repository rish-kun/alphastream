...
class TestSentimentOverview:
    async def test_returns_overview(self, client: AsyncClient, mock_db: AsyncMock):
        ...
        # 1. aggregate sentiment and counts -> (0.35, 10, 5, 15)
        # 2. top movers -> empty list
        mock_db.execute.side_effect = [
            MockResult(data=[(0.35, 10, 5, 15)]),  # combined aggregate query
    ...
    async def test_overview_with_null_sentiment(
        ...
        mock_db.execute.side_effect = [
            MockResult(data=[(None, 0, 0, 0)]),  # combined aggregate query
            MockResult(data=[]),  # top movers
        ]
...
class TestSentimentReanalysis:
    async def test_reanalyze_dispatches_existing_articles(
        ...
        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.api.v1.sentiment.reanalysis_status_service.start_reanalysis", new_callable=AsyncMock):
