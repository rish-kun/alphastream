...
class PortfolioService:
    ...
    async def get_portfolio_news(
        ...
        # ⚡ Bolt: Using EXISTS (.any) prevents duplicate rows when an article mentions
        # multiple stocks, avoiding Python-side .unique() and ensuring .limit(50) is exact
        articles_stmt = (
            select(NewsArticle)
            .where(
                NewsArticle.mentions.any(ArticleStockMention.stock_id.in_(stock_ids))
            )
