...
class StockService:
    ...
    async def get_stock_alpha(self, ticker: str) -> dict:
        ...
        # Use PostgreSQL DISTINCT ON (via SQLAlchemy .distinct()) to deduplicate at the DB level.
        # This optimization reduces memory overhead and data transfer by fetching only
        # the single most recent record per window_hours, rather than all historical records.
        metrics_stmt = (
            select(AlphaMetric)
