...
class StockService:
    ...
    async def get_stock_alpha(self, ticker: str) -> dict:
        ...
        # ⚡ BOLT OPTIMIZATION: Push deduplication to PostgreSQL using DISTINCT ON
        # Impact: O(1) memory instead of O(N) where N could be thousands of rows
        # Why: PostgreSQL handles deduplication much more efficiently before returning rows over the network
        metrics_stmt = (
            select(AlphaMetric)
