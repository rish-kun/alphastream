...
class StockService:
    ...
    async def get_stock_alpha(self, ticker: str) -> dict:
        ...
        # ⚡ Bolt Optimization: Use PostgreSQL's DISTINCT ON to deduplicate at the database level.
        # This prevents pulling thousands of historical metrics over the wire only to discard them in Python.
        # Expected Impact: Reduces database payload size by 90%+ and eliminates O(n) loop overhead for heavily traded stocks.
        metrics_stmt = (
            select(AlphaMetric)
