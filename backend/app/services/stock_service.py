...
class StockService:
    ...
    async def get_stock_alpha(self, ticker: str) -> dict:
        ...
        # Use PostgreSQL's DISTINCT ON (via .distinct()) to fetch only the most recent
        # metric per window directly at the DB level, avoiding pulling all history into Python memory.
        metrics_stmt = (
            select(AlphaMetric)
