...
class NewsService:
    ...
    async def get_news_feed(self, query: NewsFeedQuery) -> NewsListResponse:
        ...
        if query.ticker:
            # ⚡ Bolt: Use .any() and .has() to generate EXISTS subquery instead of JOIN to prevent duplicate rows
            # and avoid the need for .distinct() and its associated performance overhead
            conditions.append(
                NewsArticle.mentions.any(
                    ArticleStockMention.stock.has(
