...
class NewsService:
    ...
    async def get_news_feed(self, query: NewsFeedQuery) -> NewsListResponse:
        ...
        if query.ticker:
            # ⚡ Bolt: Use EXISTS (.any/.has) instead of JOIN + DISTINCT
            # to filter by ticker. This avoids duplicating rows in the result set
            # and eliminates the need for an expensive distinct() sort in postgres.
            conditions.append(
                NewsArticle.mentions.any(
                    ArticleStockMention.stock.has(
