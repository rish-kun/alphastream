...
async def get_market_sentiment(
    ...
    # ⚡ Bolt: Optimize by aggregating average and counts in a single database query.
    # We use conditional aggregation (COUNT(CASE WHEN condition THEN 1 END)) to combine
    # 4 separate database roundtrips into a single pass over the SentimentAnalysis table.
    agg_stmt = select(
