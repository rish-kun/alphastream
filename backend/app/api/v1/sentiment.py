...
async def get_market_sentiment(
    ...
    # Calculate average sentiment and counts (bullish, bearish, neutral) in a single query
    # ⚡ Bolt: Optimizing multiple aggregate queries into one to reduce DB round-trips
    stats_stmt = (
        select(
    ...
    market_sentiment = float(stats_row[0]) if stats_row and stats_row[0] is not None else 0.0
    bullish_count = stats_row[1] if stats_row and stats_row[1] is not None else 0
    bearish_count = stats_row[2] if stats_row and stats_row[2] is not None else 0
    neutral_count = stats_row[3] if stats_row and stats_row[3] is not None else 0
