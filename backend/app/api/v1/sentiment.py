...
async def get_market_sentiment(
    ...
    # Bolt Optimization: Combine 4 separate queries (avg, bullish, bearish, neutral counts)
    # into a single database query using conditional aggregation, significantly reducing
    # database round-trips from 4 to 1.
    stmt = select(
        ...
        func.count(case((
            and_(SentimentAnalysis.sentiment_score >= -0.3, SentimentAnalysis.sentiment_score <= 0.3), 1
        ))).label("neutral_count"),
    ...
    market_sentiment = float(row[0]) if row and row[0] is not None else 0.0
    bullish_count = row[1] if row and row[1] is not None else 0
    bearish_count = row[2] if row and row[2] is not None else 0
    neutral_count = row[3] if row and row[3] is not None else 0
