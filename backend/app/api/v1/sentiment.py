...
async def get_market_sentiment(
    ...
    # ⚡ Bolt Optimization: Combine average sentiment and category counts into a single query
    # using conditional aggregation to reduce database round-trips from 4 to 1.
    stmt = (
        select(
            func.avg(SentimentAnalysis.sentiment_score).label("avg_sentiment"),
            func.count(case((SentimentAnalysis.sentiment_score > 0.3, 1))).label("bullish"),
            func.count(case((SentimentAnalysis.sentiment_score < -0.3, 1))).label("bearish"),
            func.count(
                case(
                    (
                        and_(
    ...
    if row:
        market_sentiment = float(row[0]) if row[0] is not None else 0.0
