...
async def get_market_sentiment(
    ...
    # Get average sentiment and counts (bullish, bearish, neutral) in a single query
    agg_stmt = (
        select(
    ...
    bullish_count = int(row[1]) if row and row[1] is not None else 0
    bearish_count = int(row[2]) if row and row[2] is not None else 0
    neutral_count = int(row[3]) if row and row[3] is not None else 0
