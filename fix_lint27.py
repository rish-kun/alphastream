...
content = content.replace('''    async def test_reanalyze_dispatches_existing_articles(
        ...
        assert resp.status_code == 202''', '''    async def test_reanalyze_dispatches_existing_articles(
        self, client: AsyncClient, mock_db: AsyncMock
    ):
        article_id = uuid.uuid4()
        mock_db.execute.return_value = MockResult(data=[article_id])

        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.api.v1.sentiment.reanalysis_status_service.start_reanalysis") as mock_start:
            mock_send_task.return_value = MagicMock(id="task-123")
...
content = content.replace('''    async def test_reanalyze_all_dispatches_job(
        ...
        assert resp.status_code == 202''', '''    async def test_reanalyze_all_dispatches_job(
        self, client: AsyncClient, mock_db: AsyncMock
    ):
        with patch("app.api.v1.sentiment._celery_app.send_task") as mock_send_task, patch("app.api.v1.sentiment.reanalysis_status_service.start_reanalysis") as mock_start:
            mock_send_task.return_value = MagicMock(id="task-all")
